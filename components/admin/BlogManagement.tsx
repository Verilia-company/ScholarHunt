...
export default function BlogManagement() {
      ...
      try {
        ...
        if (user && userProfile) {
          await analyticsService.logActivity({
            ...
            item: posts.find((p) => p.id === id)?.title || "Unknown post",
            resourceType: "blog",
            resourceId: id,
          });
        }
  ...
  const handleToggleFeatured = async (id: string) => {
    try {
      ...
      await blogService.updateBlogPost(id, { featured: newFeatured });

      setPosts(
        posts.map((post) =>
          post.id === id ? { ...post, featured: newFeatured } : post
        )
      );
    } catch (error) {
  ...
  const handleAddPost = async (
    newPostData: Omit<BlogPost, "id" | "createdAt" | "updatedAt">
  ) => {
    try {
      setIsSubmitting(true);

      // Create the blog post in Firebase
      const postId = await blogService.createBlogPost(newPostData);

      // Get the created post to update local state
      const createdPost = await blogService.getBlogPost(postId);
      if (createdPost) {
      ...
      if (user && userProfile) {
        await analyticsService.logActivity({
          ...
          userName: userProfile.displayName || userProfile.email || "Unknown",
          action:
            newPostData.status === "published"
              ? "Blog post published"
              : "Blog post created",
          item: newPostData.title,
          resourceType: "blog",
          resourceId: postId,
        });
      }
  ...
  const handleEditPost = async (
    ...
    try {
      ...
      await blogService.updateBlogPost(editingPost.id, updatedPostData);

      // Get the updated post to update local state
      const updatedPost = await blogService.getBlogPost(editingPost.id);
      if (updatedPost) {
        setPosts(
      ...
      if (user && userProfile) {
        await analyticsService.logActivity({
          ...
          item: updatedPostData.title,
          resourceType: "blog",
          resourceId: editingPost.id,
        });
      }
  ...
  const PostForm = ({
    ...
    const [formData, setFormData] = useState({
      ...
      author:
        post?.author || userProfile?.displayName || userProfile?.email || "",
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200 sticky bottom-0 bg-white">
              ...
              <button
                ...
                className="w-full sm:w-auto btn-primary flex items-center justify-center gap-2 text-sm sm:text-base disabled:opacity-50 disabled:cursor-not-allowed"
              >
