...
// Dummy categories for dropdown (replace with your actual categories)
const categories = [
  "Tips & Guides",
  "Opportunities",
  "Success Stories",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl p-6 overflow-y-auto max-h-[90vh]">
        <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {formData.socialImage && (
                    <div className="mt-2 relative">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setFormData({ ...formData, socialImage: "" })
                        }
                        className="absolute top-2 right-2 bg-red-600 text-white rounded-full p-1 hover:bg-red-700"
                        title="Remove image"
                      >
          ...
          {/* Form Actions */}
          <div
            className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200 sticky bottom-0"
            style={{
...
export default function BlogManagement() {
  ...
  const handleAddPost = async (formData: BlogPost) => {
    ...
    try {
      ...
      if (typeof result === "string") {
        // If result is an ID, fetch the full post
        const fetchedPost = await blogService.getBlogPost(result);
        if (!fetchedPost) {
          throw new Error("Failed to fetch the newly created blog post.");
        }
        newPost = fetchedPost;
      } else {
        newPost = result;
  ...
  const handleTogglePublished = async (id: string) => {
    try {
      ...
      await blogService.updateBlogPost(id, { status: newStatus });
      setPosts(
        posts.map((p) => (p.id === id ? { ...p, status: newStatus } : p))
      );
    } catch (error) {
  ...
  const handleToggleFeatured = async (id: string) => {
    try {
      ...
      setPosts(
        posts.map((p) => (p.id === id ? { ...p, featured: newFeatured } : p))
      );
    } catch (error) {
